...
import warnings
from my_split_bootstrap import MyBootstrapSplitOnce, MyBootstrapSplitRepeated
...
list_func_tt_split = \
    [
        ...
        #   (repeated_stratified_holdout, (1.0/2.0, 4, seed)),
        #   (fold_split, (3, seed)),
        ...
        #    (bootstrap_split_repeated, (3, seed))
    ]
...
def ex2():
    ...
    dataset = load_dataset(fileName, featureName, lambda: None)  # EX2.B
    dataset = load_dataset(fileName, featureName, func_datasetLoader)  # EX2.D


def analyzeFuns(f_tt, test_fraction, dataset_param ):
    args = (test_fraction, seed)

    (X, y, tt_split_indexes) = train_test_split_recipe(dataset_param, f_tt, *args)
...
def ex3():
    ...
    dataset = load_dataset(fileName, featureName, func_datasetLoader)

    holdout(dataset)  # EX3.A
    analyzeFuns(holdout, 1 / 2, dataset) #EX3.B
    analyzeFuns(stratified_holdout, 1 / 2, dataset)#EX3.C
    analyzeFuns(repeated_stratified_holdout, 1 / 2, dataset)#EX3.D
